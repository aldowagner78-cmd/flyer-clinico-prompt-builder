...
function sanitizePromptOptions(promptOptions) {
  return {
    ...
    videoCreationMode: stringFrom(promptOptions.videoCreationMode) || 'Desde cero',
    videoDestination: stringFrom(promptOptions.videoDestination) || 'Instagram / WhatsApp vertical 9:16',
    videoDuration: stringFrom(promptOptions.videoDuration) || 'Automático recomendado',
    videoMotionStyle: stringFrom(promptOptions.videoMotionStyle) || 'Suave profesional',
    videoMusic: stringFrom(promptOptions.videoMusic) || 'Instrumental suave',
    videoStructure: stringFrom(promptOptions.videoStructure) || 'Beneficio → Servicio → Mensaje final',
    videoFinalMessage: stringFrom(promptOptions.videoFinalMessage) || 'Pedí tu turno',
    videoVoiceOver: stringFrom(promptOptions.videoVoiceOver) || 'Sin voz en off',
    ...
    jingleInstrumentation: stringFrom(promptOptions.jingleInstrumentation) || 'Instrumental corporativo',
    jingleIncludeSlogan: booleanFrom(promptOptions.jingleIncludeSlogan, true),
